...
The system SHALL validate the provider team's category against the referenced API category policy before provisioning an `ApiExposure`.
...
The system SHALL validate the subscriber team's category against the referenced API category policy before provisioning an `ApiSubscription`.
...
The system SHALL apply the same API-category policy semantics in both `ApiExposure` and `ApiSubscription` flows to avoid governance divergence.
