...
func main() {
	...
	webhookServer, webhookCertWatcher := setupWebhookServer(tlsOpts, webhookCertPath, webhookCertName, webhookCertKey)
	metricsServerOptions, metricsCertWatcher := setupMetricsServer(metricsAddr, secureMetrics, tlsOpts, metricsCertPath, metricsCertName, metricsCertKey)
	...
	setupLog.Info("Loaded expiration config",
		...
		"thresholds", expirationConfig.DefaultThresholds,
	)
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		webhookConfig, err := config.LoadWebhookConfig()
		if err != nil {
			setupLog.Error(err, "unable to load webhook config")
			os.Exit(1)
		}
		if err = webhookv1.SetupApprovalWebhookWithManager(mgr, webhookConfig.OperatorServiceAccount); err != nil {
...
func setupWebhookServer(tlsOpts []func(*tls.Config), certPath, certName, certKey string) (webhook.Server, *certwatcher.CertWatcher) {
...
func setupMetricsServer(metricsAddr string, secureMetrics bool, tlsOpts []func(*tls.Config), certPath, certName, certKey string) (metricsserver.Options, *certwatcher.CertWatcher) {
	...
	return metricsServerOptions, watcher
}
