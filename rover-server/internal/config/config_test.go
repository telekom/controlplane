...
func writeTempConfig(yaml string) string {
	f, err := os.CreateTemp("", "rover-config-*.yaml")
...
var _ = Describe("LoadConfig", func() {
	It("fails without deployment-provided jwt issuers", func() {
		// The external JWT listener has no trustedIssuers until deployment
		// supplies them; loading the bare default must fail-closed.
