...
var _ = Describe("Subscription Util", func() {
	Context("HasM2M", func() {
		It("should return true if SubscriberMachine2MachineAuthentication exists", func() {
			...
			Expect(HasM2M(subscription)).To(BeFalse())
			...
			Expect(HasM2M(subscription)).To(BeTrue())
		})
	})
	...
	Context("HasM2MClient", func() {
		It("should return true if SubscriberMachine2MachineAuthentication exists", func() {
			...
			subscription := &apiapi.ApiSubscription{}
			Expect(HasM2MClient(subscription)).To(BeFalse())
			...
			Expect(HasM2MClient(subscription)).To(BeTrue())
		})
	})
})
