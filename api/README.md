...
Here is a list of notable conversions within this domain:
...
- For the `allowedTeams` field, the name of the team allowed to register an API with this category. Use '*' to allow all teams to expose APIs with this category.
- For The `LabelValue` field, the name of the API category in the OpenAPI specification. APIs fall into that category if the `info.x-api-category` field of the OpenAPI spec matches the value.
</details>
<br />
