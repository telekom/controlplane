...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "30720559.cp.ei.telekom.de",
	...
	if err := (&controller.PublisherReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "Publisher")
		os.Exit(1)
	}
	if err := (&controller.SubscriberReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "Subscriber")
		os.Exit(1)
	}
