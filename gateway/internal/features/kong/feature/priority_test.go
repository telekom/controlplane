...
var _ = Describe("Feature Priority Ordering", func() {
	...
	DescribeTable("assigns correct absolute priority values to all features",
		func(f features.KongFeature, expectedPriority int) {
			Expect(f.Priority()).To(Equal(expectedPriority))
		},
		Entry("PassThrough has priority 0",
			features.KongFeature(feature.InstancePassThroughFeature), 0),
		Entry("HeaderTransformation has priority 0",
			features.KongFeature(feature.InstanceHeaderTransformationFeature), 0),
		Entry("ExternalIDP has priority 9",
			features.KongFeature(feature.InstanceExternalIDPFeature), 9),
		Entry("AccessControl has priority 10",
			features.KongFeature(feature.InstanceAccessControlFeature), 10),
		Entry("CustomScopes has priority 10",
			features.KongFeature(feature.InstanceCustomScopesFeature), 10),
		Entry("Claims has priority 10",
			features.KongFeature(feature.InstanceClaimsFeature), 10),
		Entry("RateLimit has priority 10",
			features.KongFeature(feature.InstanceRateLimitFeature), 10),
		Entry("BasicAuth has priority 10",
			features.KongFeature(feature.InstanceBasicAuthFeature), 10),
		Entry("IpRestriction has priority 10",
			features.KongFeature(feature.InstanceIpRestrictionFeature), 10),
		Entry("LastMileSecurity has priority 100",
			features.KongFeature(feature.InstanceLastMileSecurityFeature), 100),
		Entry("DynamicUpstream has priority 101",
			features.KongFeature(feature.InstanceDynamicUpstreamFeature), 101),
		Entry("LoadBalancing has priority 102",
			features.KongFeature(feature.InstanceLoadBalancingFeature), 102),
		Entry("Failover has priority 109",
			features.KongFeature(feature.InstanceFailoverFeature), 109),
		Entry("CircuitBreaker has priority 110",
			features.KongFeature(feature.InstanceCircuitBreakerFeature), 110),
	)
	...
	It("sorts features correctly when using sortFeatures-like logic", func() {
		allFeatures := []features.KongFeature{
