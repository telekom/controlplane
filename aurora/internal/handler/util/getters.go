...
func FindActiveMcpServer(ctx context.Context, basePath string) (bool, *aurorv1.McpServer, error) {
	c := cclient.ClientFromContextOrDie(ctx)

	serverList := &aurorv1.McpServerList{}
	if err := c.List(ctx, serverList, client.MatchingLabels{
		aurorv1.McpBasePathLabelKey: labelutil.NormalizeLabelValue(basePath),
	}); err != nil {
	...
	var candidates []aurorv1.McpServer
...
func FindMcpExposures(ctx context.Context, basePath string) ([]aurorv1.McpExposure, error) {
	c := cclient.ClientFromContextOrDie(ctx)

	exposureList := &aurorv1.McpExposureList{}
	...
	var exposures []aurorv1.McpExposure
...
func FindActiveMcpExposure(exposures []aurorv1.McpExposure) (bool, *aurorv1.McpExposure, error) {
	if len(exposures) == 0 {
		return false, nil, nil
	}

	var candidates []aurorv1.McpExposure
...
func FindCrossZoneMcpSubscriptionZones(ctx context.Context, basePath, exposureZoneName string) ([]ctypes.ObjectRef, error) {
	...
	subList := &aurorv1.McpSubscriptionList{}
