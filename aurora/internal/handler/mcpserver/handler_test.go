...
func newMcpServer(name, basePath string, uid types.UID, creationTime time.Time) *aurorv1.McpServer {
	return &aurorv1.McpServer{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				aurorv1.McpBasePathLabelKey: basePath,
			},
		},
	...
	Describe("CreateOrUpdate", func() {
		...
		It("should set Active=true when no other McpServer exists for basePath", func() {
			...
			fakeClient.EXPECT().
				...
				Run(func(_ context.Context, list client.ObjectList, _ ...client.ListOption) {
					*list.(*aurorv1.McpServerList) = aurorv1.McpServerList{
						Items: []aurorv1.McpServer{*obj},
					}
				}).
				Return(nil)
		...
		It("should set Active=false when another older McpServer exists for same basePath", func() {
			...
			fakeClient.EXPECT().
				...
				Run(func(_ context.Context, list client.ObjectList, _ ...client.ListOption) {
					*list.(*aurorv1.McpServerList) = aurorv1.McpServerList{
						Items: []aurorv1.McpServer{*existing, *obj},
					}
				}).
				Return(nil)
		...
		It("should set Active=true when it is the oldest McpServer for basePath", func() {
			...
			fakeClient.EXPECT().
				...
				Run(func(_ context.Context, list client.ObjectList, _ ...client.ListOption) {
					*list.(*aurorv1.McpServerList) = aurorv1.McpServerList{
						Items: []aurorv1.McpServer{*newer, *obj},
					}
				}).
				Return(nil)
		...
		It("should ignore McpServers with different basePaths", func() {
			...
			fakeClient.EXPECT().
				...
				Run(func(_ context.Context, list client.ObjectList, _ ...client.ListOption) {
					*list.(*aurorv1.McpServerList) = aurorv1.McpServerList{
						Items: []aurorv1.McpServer{*different, *obj},
					}
				}).
				Return(nil)
