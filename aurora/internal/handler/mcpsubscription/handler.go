...
import (
	...
	"github.com/telekom/controlplane/aurora/internal/handler/util"
...
var _ handler.Handler[*aurorv1.McpSubscription] = &McpSubscriptionHandler{}

type McpSubscriptionHandler struct{}

//nolint:gocyclo // reconciler with sequential validation, approval, and provisioning steps
func (h *McpSubscriptionHandler) CreateOrUpdate(ctx context.Context, obj *aurorv1.McpSubscription) error {
...
func (h *McpSubscriptionHandler) Delete(ctx context.Context, obj *aurorv1.McpSubscription) error {
	// Child resources (ConsumeRoute, ApprovalRequest) are cleaned up via owner references.
	return nil
}
...
func (h *McpSubscriptionHandler) createConsumeRoute(
	...
	mutator := func() error {
		consumeRoute.Labels = map[string]string{
			...
			aurorv1.McpBasePathLabelKey:     labelutil.NormalizeLabelValue(obj.Spec.McpBasePath),
...
func validateVisibility(exposure *aurorv1.McpExposure, sub *aurorv1.McpSubscription, subscriberZone *adminv1.Zone) bool {
	switch exposure.Spec.Visibility {
	case aurorv1.VisibilityZone:
		// Only same-zone subscriptions
		return sub.Spec.Zone.Name == exposure.Spec.Zone.Name
	case aurorv1.VisibilityEnterprise:
		// Same enterprise (currently no cross-enterprise check, allow all)
		return true
	case aurorv1.VisibilityWorld:
		return true
	default:
		return false
	}
