...
### Leader Election ID
- Pattern: Uses truncated hash + domain format
- Example: `58816de6.cp.ei.telekom.de`
...
- `cp.ei.telekom.de7application` - Application reference
...
### Kubebuilder
- **Required Version**: 4.9.0 (pinned)
- **Release**: https://github.com/kubernetes-sigs/kubebuilder/releases/tag/v4.9.0
...
### Tool Versions (pinned in Makefile)
```makefile
...
### Linting
Uses **golangci-lint** with specific enabled linters (`.golangci.yml`):
...
### Exclusions
Generated code and third-party code are excluded from linting:
```yaml
exclusions:
  paths:
    - third_party$
...
### Go Modules
- Each domain has independent `go.mod`
