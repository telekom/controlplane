...
<InfoSection type="note" title="Kustomize components">
  The key components of Kustomize configurations used in the Control Plane.
</InfoSection>

### kustomization.yaml

The main configuration file that defines how resources are customized:
...
# Configuration for generating ConfigMaps
configMapGenerator:
...
<InfoSection type="tip" title="Layered approach">
  The Control Plane uses a base + overlay approach to customize manifests for different environments while maintaining consistency.
</InfoSection>

### Base Configuration

Base configurations define the core resources that are common across all environments:
...
<CardGrid columns={2}>
  <FeatureCard
    title="Strategic Merge Patches"
    description={<>
      <p>Merge-based patches for modifying existing resources:</p>
      <code>
      # increase-replicas.yaml<br/>
  ...
  <FeatureCard
    title="JSON Patches"
    description={<>
      <p>JSON patch operations for precise modifications:</p>
      <code>
      # set-resource-limits.yaml<br/>
      - op: replace<br/>
      &nbsp;&nbsp;path: /spec/template/spec/containers/0/resources/limits/memory<br/>
      &nbsp;&nbsp;value: 512Mi<br/>
      - op: replace<br/>
      &nbsp;&nbsp;path: /spec/template/spec/containers/0/resources/limits/cpu<br/>
      &nbsp;&nbsp;value: 500m
      </code>
    </>}
  />
...
## Practical Examples from Control Plane

<InfoSection type="note" title="Real-world usage">
  Examples of how Kustomize is used in the Control Plane for different deployment scenarios.
</InfoSection>

### Controller Manager Deployment

The base deployment for a controller manager:
...
images:
- name: controller
  newName: registry.controlplane.example.com/controller
  newTag: v1.2.3
...
<InfoSection type="tip" title="Operator deployment">
  The workflow for deploying Control Plane operators using Kustomize.
</InfoSection>

```mermaid
graph TD
    A[Edit Base Resources] --> B[Create Environment Overlays]
    B --> C[Build Kustomization]
    C --> D{Apply Directly?}
    D -->|Yes| E[kubectl apply -k]
    D -->|No| F[kubectl kustomize > manifests.yaml]
    F --> G[Apply Generated Manifests]
    
    classDef primary fill:#E20074,color:#ffffff,stroke:#E20074
    classDef secondary fill:#00ADD8,color:#ffffff,stroke:#00ADD8
...
## Kubebuilder-Generated Structure

<InfoSection type="note" title="Generated configurations">
  Kubebuilder generates a specific Kustomize structure for operators in the Control Plane.
</InfoSection>

### Default Configuration

The default kustomization combines CRDs, RBAC, and the manager:
...
# ConfigMap for the controller manager
configMapGenerator:
- name: manager-config
  files:
...
Custom Resource Definitions are kept separate to allow installation without the controller:
...
resources:
- bases/storage.cp.ei.telekom.de_filemanagers.yaml
```
...
<InfoSection type="note" title="Automated deployments">
  How Kustomize integrates with the Control Plane CI/CD pipelines.
</InfoSection>
...
graph TD
    ...
    D --> E[Update Image in Kustomize]
    E --> F[Generate Manifests]
...
```yaml
# Update image and deploy to dev environment
- name: Update kustomization
  run: |
    cd config/overlays/development
    kustomize edit set image controller=registry.controlplane.example.com/controller:${GITHUB_SHA::7}
...
- name: Wait for rollout
  run: |
    kubectl rollout status deployment/controller-manager -n development --timeout=180s
```
  ...
  <FeatureCard
    ...
    description={<>
      ...
      <ul>
        ...
        <li>Complex deployments with conditionals</li>
        <li>User-facing components</li>
      </ul>
    </>}
  />
</CardGrid>
...
<CardGrid columns={2}>
  ...
  <FeatureCard
    title="Kubernetes"
    description="Understand the platform that Kustomize configures."
