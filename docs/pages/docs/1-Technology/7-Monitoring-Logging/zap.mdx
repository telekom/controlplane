...
```mermaid
graph LR
    A[Application Code] --> B[Zap Logger]
    B --> C[zapr Adapter]
    C --> D[logr Interface]
    ...
    classDef app fill:#E20074,color:#ffffff,stroke:#E20074
    classDef log fill:#00ADD8,color:#ffffff,stroke:#00ADD8
    classDef infra fill:#2C3E50,color:#ffffff,stroke:#2C3E50
    classDef viz fill:#f9f9f9,color:#333,stroke:#ccc
...
<CardGrid columns={3}>
  <FeatureCard
    ...
    description={
      <>
        <p><strong>Info level and above</strong></p>
        <ul>
          <li>Info: Normal operations</li>
          <li>Warn: Potential issues</li>
          <li>Error: Failures requiring attention</li>
          <li>DPanic: Critical errors in development</li>
          <li>Panic: Critical errors that log and panic</li>
          <li>Fatal: Critical errors that log and exit</li>
        </ul>
      </>
    }
  />
  ...
  <FeatureCard
    ...
    description={
      <>
        <p><strong>Debug level and above</strong></p>
        <ul>
          <li>Debug: Fine-grained information</li>
          <li>Info: Normal operations</li>
          <li>Warn: Potential issues</li>
          <li>Error: Failures requiring attention</li>
          <li>DPanic: Critical errors that panic in development</li>
          <li>Panic: Critical errors that log and panic</li>
          <li>Fatal: Critical errors that log and exit</li>
        </ul>
      </>
  ...
  <FeatureCard
    ...
    description={
      <>
        ...
        <ul>
          <li>Can be adjusted per test suite</li>
          <li>Often uses Warn and above to reduce noise</li>
          <li>Can send to a memory buffer for assertions</li>
          <li>Supports discarding output entirely</li>
          <li>Can capture logs for test verification</li>
        </ul>
      </>
    }
  />
...
```go
// Create an AtomicLevel to manage the log level
atomicLevel := zap.NewAtomicLevel()
atomicLevel.SetLevel(zap.InfoLevel)

// Create a logger with the atomic level
config := zap.Config{
    Level:            atomicLevel,
    Encoding:         "json",
...
// Later, adjust the level dynamically
atomicLevel.SetLevel(zap.DebugLevel)  // Temporarily increase verbosity
// ... after troubleshooting
atomicLevel.SetLevel(zap.InfoLevel)   // Return to normal level
```
...
{`{
  ...
  "caller": "controller/request.go:42",
...
<CardGrid columns={1}>
  <FeatureCard
    ...
    description={
      <>
        <p>Performance comparison on Control Plane logging workload:</p>
        <table>
          <thead>
            <tr>
              <th>Logger</th>
        ...
        <p><em>Lower values are better. Benchmark with 10 fields per log entry.</em></p>
      </>
    }
  />
</CardGrid>
