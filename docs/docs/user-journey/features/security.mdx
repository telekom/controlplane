...
You define the audience under `security.m2m.claims.aud`. You must set **exactly one** of the following:

- `value` — a fixed text value that you choose.
- `valueFrom` — a predefined source that the platform fills in for you.

:::caution
The audience claim rides only on the platform-managed token. It cannot be combined with an external identity provider (`security.m2m.externalIDP`) or basic authentication (`security.m2m.basic`) — the file is rejected when you apply it. You may use it on its own or alongside `security.m2m.scopes`.
:::
...
| Field | Description |
| ----- | ----------- |
| `security.m2m.claims.aud.value` | A fixed audience value that you provide. Cannot be used together with `valueFrom`. |
| `security.m2m.claims.aud.valueFrom` | A predefined source the platform resolves into the audience value. One of `ProviderClientId`, `BasePath`, or `ConsumerClientId`. Cannot be used together with `value`. |
