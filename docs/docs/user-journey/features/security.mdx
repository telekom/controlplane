...
#### Scope Validation and External Identity Providers

For API and agentic (MCP and agent) exposures and subscriptions, configured M2M scopes normally must be declared in the corresponding specification. Matching is exact and case-sensitive.

When the **active exposure has a non-empty external-IDP token endpoint**, the Control Plane skips this specification check for both the exposure and its subscriptions. Scopes may then be absent from the specification, including when it declares no scopes at all. Each exposure and subscription continues to forward its own configured scopes; subscription scopes do not have to match the exposure's list.

In a Rover exposure, configure `security.tokenEndpoint` as shown in the OAuth2 example above. On the resulting `ApiExposure` or `AgenticExposure` custom resource, the relevant field is `spec.security.m2m.externalIDP.tokenEndpoint`:

```yaml title="Exposure custom-resource fragment"
spec:
  security:
    m2m:
      externalIDP:
...
The subscription inherits this exception from its active exposure. Subscriber credentials or a token endpoint configured elsewhere do not enable it. Removing the exposure's external-IDP configuration restores normal scope checking; an empty endpoint does not qualify for the exception.

API approval requests still include the requested scopes. Existing readiness and approval requirements continue to apply, and the external identity provider determines whether it will grant the requested scopes.
...
| ------ | ------- | -------------------- |
| `PROVIDER_CLIENT_ID` | The identifier of your application (the API provider). | Automatically, by the platform. |
| `BASE_PATH` | The base path of the exposed API. | Automatically, by the platform. |
| `CONSUMER_CLIENT_ID` | The identifier of whichever application is calling the API. | At the moment of each request, because it is different for every consumer. |
...
| `security.claims.aud.valueFrom` | A predefined source the platform resolves into the audience value. One of `PROVIDER_CLIENT_ID`, `BASE_PATH`, or `CONSUMER_CLIENT_ID`. Cannot be used together with `value`. |
