...
| Field | Description |
| ----- | ----------- |
| `type` | A globally unique identifier for the event type. Lowercase letters and digits form nonempty segments separated by single dots or hyphens (e.g., `de.telekom.orders-created.v1`). It must end in `.v<major>`, matching the major number in `version`. |
...
| Field | Description |
| ----- | ----------- |
| `eventType` | The event type identifier. Uses the same lowercase letter/digit segments and dot/hyphen separators as `type`; must match an existing EventType resource registered in Step 1. |
