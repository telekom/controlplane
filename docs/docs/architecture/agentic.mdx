...
The AI Gateway is a regular zone gateway that declares `AI` in its `types`:
...
A single gateway may serve several types, so `types: [API, AI]` is a valid way to run agentic and API traffic on one gateway. The Agentic operator resolves its routes through the preset that references an `AI` gateway; a zone without one rejects agentic exposures and subscriptions during Rover validation. See [Environments and Zones](../admin-journey/environments-and-zones.md#gateway-types) for the full gateway type reference.
