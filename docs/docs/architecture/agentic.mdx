...
URL and the subscriber zone's identity-provider issuer in its status. The
Projector persists both values and exposes them as `gatewayURL` and `idpIssuer`
on the GraphQL `AgenticSubscription` type.
