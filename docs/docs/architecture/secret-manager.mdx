...
:::info
When Secret-Manager is **disabled** in an installation, the same zone secrets are generated **inline** and kept on the resource instead of being onboarded. See [Admin Domain: Secret onboarding](./admin.mdx#secret-onboarding-defaulting-webhook) for the zone-specific behavior.
:::
