...
For Listeners, approvals, and approval requests, isolation uses an **OR** rule: a resource is visible if the caller's team is either the subscriber's team or the exposure owner's team. This allows both sides to inspect the Listener and its approval workflow.

`ApiExposure.listeners` and `ApiSubscription.listeners` return only Listeners whose projected `Ready` condition is true. `Application.listeners` and top-level Listener queries include non-ready Listeners. API-specific values such as `apiBasePath`, `requestFilter`, and `responseFilter` are flat Listener fields. `approved` is true only when the provider approval is granted. Monitoring state is not stored or exposed; clients derive it from projected state.
