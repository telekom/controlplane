...
The **default mode is `jwt`** (secure by default). A deployment without explicit security configuration will panic at startup if no `trustedIssuers` are provided — this is intentional fail-closed behaviour.
