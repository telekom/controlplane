...
erDiagram
    ...
    Application ||--o{ Listener : "declares"
    ApiExposure ||--o{ ApiSubscription : "target of"
    ApiSubscription ||--o| Approval : "has"
    ApiSubscription ||--o| ApprovalRequest : "has"
    ApiSubscription ||--o{ Listener : "consumed by"
    ApiExposure ||--o{ Listener : "provided by"
    Listener ||--o| Approval : "provider approval"
    Listener ||--o{ ApprovalRequest : "has"
```
...
- `listeners` lists API Listeners, including non-ready Listeners
...
For Listeners, approvals, and approval requests, isolation uses an **OR** rule: a resource is visible if the caller's team owns the listener application, the consumer application, or the provider application. This allows all three parties to inspect the Listener and its approval workflow.

`Listener.application` identifies the application that declared the Listener and receives captured events. `Listener.consumer` and `Listener.provider` are derived from the related API subscription and exposure. `Application.listeners` follows the listener-application relationship, not the consumer relationship.

`ApiExposure.listeners` and `ApiSubscription.listeners` return only Listeners whose projected `Ready` condition is true. `Application.listeners` and top-level Listener queries include non-ready Listeners. API-specific values such as `apiBasePath`, `requestFilter`, and `responseFilter` are flat Listener fields. `approved` is true only when both the provider and consumer approvals are granted. `providerApproval` and `consumerApproval` expose the separate decisions. Monitoring state is not stored or exposed; clients derive it from projected state.
