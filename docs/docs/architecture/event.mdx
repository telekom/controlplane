...
To move events between the people who send them, the people who receive them, and the event backend (Horizon), the Event domain wires up four kinds of gateway routes. A route is simply an address on the gateway that accepts traffic and forwards it to the right place.

Three of these routes handle the everyday flow of events and are created **once per zone**, the moment the event feature is switched on in that zone. The fourth is created **once per event type**, when a provider exposes an event.

| Route | Created | Used by | What it does |
|-------|---------|---------|--------------|
...
Every one of these routes comes in two flavours. When the sender and receiver share the same zone, a single **local route** connects them directly. When they live in different zones, the platform adds a **proxy route** in the caller's own zone that quietly forwards the traffic across the zone boundary — so each participant only ever talks to a gateway in its own zone, and the cross-zone hop stays invisible to them.
...
```mermaid
graph LR
    subgraph Local["Same zone"]
        P1["<b>Publisher</b>"] -->|"send events"| PR1["<b>Publish Route</b>"]
        PR1 --> B1["<b>Event Backend</b>"]
    end

    subgraph Proxy["Publisher in a proxy zone"]
        P2["<b>Publisher</b>"] -->|"send events"| PR2["<b>Publish Route</b><br/><i>proxy zone</i>"]
        PR2 -->|"cross-zone hop"| PR3["<b>Publish Route</b><br/><i>target zone</i>"]
        PR3 --> B2["<b>Event Backend</b>"]
    end
```
...
graph LR
    subgraph Local["Same zone"]
        H1["<b>Event Backend</b>"] -->|"push event"| CR1["<b>Callback Route</b>"]
        CR1 -->|"forward"| S1["<b>Subscriber's address</b>"]
    end

    subgraph Cross["Different zones"]
        H2["<b>Event Backend</b>"] -->|"push event"| CR2["<b>Callback Route</b><br/><i>backend zone</i>"]
        CR2 -->|"cross-zone hop"| CR3["<b>Callback Route</b><br/><i>subscriber zone</i>"]
        CR3 -->|"forward"| S2["<b>Subscriber's address</b>"]
    end
```
...
graph LR
    subgraph Local["Same zone"]
        U1["<b>Subscriber</b>"] -->|"request redelivery"| VR1["<b>Voyager Route</b>"]
        VR1 --> V1["<b>Voyager Backend</b>"]
    end

    subgraph Cross["Different zones"]
        U2["<b>Subscriber</b>"] -->|"request redelivery"| VR2["<b>Voyager Route</b><br/><i>subscriber zone</i>"]
        VR2 -->|"cross-zone hop"| VR3["<b>Voyager Route</b><br/><i>provider zone</i>"]
        VR3 --> V2["<b>Voyager Backend</b>"]
    end
```
...
graph LR
    subgraph Local["Same zone"]
        C1["<b>Subscriber</b>"] -->|"open stream"| SR1["<b>SSE Route</b>"]
        SR1 -->|"live events"| H1["<b>Event Backend</b>"]
    end

    subgraph Cross["Different zones"]
        C2["<b>Subscriber</b>"] -->|"open stream"| SR2["<b>SSE Route</b><br/><i>subscriber zone</i>"]
        SR2 -->|"cross-zone hop"| SR3["<b>SSE Route</b><br/><i>provider zone</i>"]
        SR3 -->|"live events"| H2["<b>Event Backend</b>"]
    end
```
