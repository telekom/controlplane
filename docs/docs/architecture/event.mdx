...
- **PubSub domain** — The Event domain creates EventStore, Publisher, and Subscriber resources. The Spectre domain also creates Publishers and Subscribers as a peer business-logic domain.
- **Spectre domain** — A peer domain that independently creates PubSub resources (Publishers and Subscribers) for application listeners. Both domains share the same EventStore, which is owned exclusively by EventConfig.
