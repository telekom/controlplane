...
func (r *FileExposureReconciler) MapFileTypeToFileExposure(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	err := r.List(ctx, list, &client.ListOptions{
		LabelSelector: util.FileTypeLabelSelector(fileType.Name),
	...
	for i := range list.Items {
		if list.Items[i].Spec.FileType == obj.GetName() {
...
func (r *FileExposureReconciler) MapZoneServiceConfigToFileExposure(ctx context.Context, obj client.Object) []reconcile.Request {
	...
	list := &filev1.FileExposureList{}
	if err := r.List(ctx, list, client.InNamespace(zoneServiceConfig.Namespace)); err != nil {
		return nil
	}

	reqs := make([]reconcile.Request, 0, len(list.Items))
	for i := range list.Items {
		if list.Items[i].Spec.Zone != nil && list.Items[i].Spec.Zone.Equals(zoneServiceConfig) {
			reqs = append(reqs, reconcile.Request{NamespacedName: client.ObjectKeyFromObject(&list.Items[i])})
		}
	}
	return reqs
