...
func (r *FileTypeReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&sftpv1.Instance{},
			handler.EnqueueRequestsFromMapFunc(r.MapSFTPInstanceToFileType),
			builder.WithPredicates(predicate.ResourceVersionChangedPredicate{}),
		).
...
func (r *FileTypeReconciler) MapSFTPInstanceToFileType(ctx context.Context, obj client.Object) []reconcile.Request {
	instance, ok := obj.(*sftpv1.Instance)
	if !ok {
		return nil
	}
	// Instance name equals the FileType name (see SFTPInstanceRefForFileExposure).
	return []reconcile.Request{{
