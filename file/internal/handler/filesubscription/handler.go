...
func (h *FileSubscriptionHandler) CreateOrUpdate(ctx context.Context, obj *filev1.FileSubscription) error {
	...
	case builder.ApprovalResultDenied:
		...
		if cleanupErr := h.deleteSubscriberUser(ctx, obj); cleanupErr != nil {
			return errors.Wrapf(cleanupErr, "unable to cleanup SFTP User for FileSubscription %q in namespace %q",
	...
	if err = h.syncSubscriberUser(ctx, obj, fileType, activeExposure); err != nil {
		return err
	}
...
func (h *FileSubscriptionHandler) ensureApproval(ctx context.Context, obj *filev1.FileSubscription, fileType *filev1.FileType, activeExposure *filev1.FileExposure) (builder.ApprovalResult, error) {
	...
	properties, err := approvalProperties(obj)
	if err != nil {
	...
	if err = requester.SetProperties(properties); err != nil {
		return builder.ApprovalResultNone, errors.Wrapf(err, "unable to set approvalRequest properties for FileSubscription %q in namespace %q",
			obj.Name, obj.Namespace)
	}
...
func approvalProperties(subscription *filev1.FileSubscription) (map[string]any, error) {
