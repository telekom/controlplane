...
var _ = Describe("Controller config loading", func() {
	Context("loadConfigFromFile -> EmptySpec", func() {
		It("merges defaults when values are missing", func() {
			cfg, err := loadConfigFromFile[EmptySpec](fixturePath("controller-defaults.yaml"))
		...
		It("returns an error when required fields are missing", func() {
			_, err := loadConfigFromFile[EmptySpec](fixturePath("controller-leader-election-invalid.yaml"))
			Expect(err).To(HaveOccurred())
		})

		It("loads reconciler overrides and derives the finalizer name", func() {
			cfg, err := loadConfigFromFile[EmptySpec](fixturePath("controller-reconciler-overrides.yaml"))
			Expect(err).ToNot(HaveOccurred())
	...
	Context("loadConfigFromFile -> sampleSpec", func() {
		It("loads the generic spec alongside common config", func() {
			cfg, err := loadConfigFromFile[sampleSpec](fixturePath("app-with-spec.yaml"))
			Expect(err).ToNot(HaveOccurred())
