...
2. Run Semantic Release
   ...
   - **Executes versioning scripts** (see Unified Versioning below), which
     commit the version-bump files locally (not pushed to `main`)
   - Core creates and pushes the `v*` git tag (which includes that commit)
...
1. **Rover-CTL Base Image** (via reusable `rover-ctl-base-image.yaml`) -
   independent of the release version; runs unconditionally as a freshness
   safety net
2. **GoReleaser** (via reusable `release-goreleaser.yaml`, also independently
...
3. **Mirror to Artifactory** (needs GoReleaser) - a matrix job, one entry
...
4. **Rover-CTL Combined Image** (needs Rover-CTL Base Image + GoReleaser)
   - Layers rover-ctl binary (from GHCR) onto bash/jq/yq base image (from Artifactory)
   - Publishes to internal Artifactory registry
5. **Release Publish Summary** - fans in all job results, always runs
...
**Permissions:** Packages (write, goreleaser job), contents (write, goreleaser job), packages (read, roverctl-combined-image job)
...
Three scripts ensure consistent versioning across install overlays and Helm charts:
...
3. **`commit_release_files.sh`**
   - Commits the two files above locally (`git add` + `git commit`), so
     they're included in the commit that semantic-release core then tags
   - Deliberately does **not** push to `main`: semantic-release core already
     pushes the tag on its own (see below), and separately pushing this
     commit to `main` would bypass required PR review, could race with
     concurrent pushes, and - since release.yaml uses a GitHub App token so
     the tag push triggers other workflows - would also spuriously
     re-trigger `ci.yaml`'s push-to-main CI run
   - Called by semantic-release during the prepare phase
...
- Update version references in install overlays and Helm charts, and commit
  them locally
- Modified files: `install/overlays/default/kustomization.yaml`, `common-server/helm/Chart.yaml`
- No `@semantic-release/git` or `@semantic-release/changelog` plugin is
  used: semantic-release *core* creates and pushes the tag by itself
  (the git plugin's only job would be pushing commits to the branch, which
  we deliberately don't do - see `commit_release_files.sh` above); the
  changelog lives in the GitHub Release generated by GoReleaser instead of
  a committed `CHANGELOG.md`
- Consequence: `main`'s copies of `kustomization.yaml`/`Chart.yaml` reflect
  the last release only until further commits land on `main` - consumers
  should always pin to a release tag, never to `main`
...
└─ Semantic Release (analyze commits, bump version-reference files, commit locally)
   └─ Push `v*` tag (core, includes that commit) ─────┐
                                                        ▼
...
```
Each Release Publish job is a pure function of the already-pushed tag, so
any of them (e.g. Artifactory mirroring, or GoReleaser itself) can be
individually re-run without cutting a new release.
