...
2. Run Semantic Release
   ...
   - Commits the version-bump files and pushes that commit to the release
     branch (`@semantic-release/git`, scoped to only those two files, no
     `CHANGELOG.md`) - required so the release tag (created next) stays
     reachable from the branch; see Unified Versioning below for why
   - Core creates and pushes the `v*` git tag, pointing at that same commit

**Permissions:** contents (write), id-token (write)

Does nothing else - no images are built or published here, and no
CHANGELOG.md is committed; the changelog lives in the GitHub Release created
by GoReleaser in release-publish.yaml below.
...
- `@semantic-release/git` then commits just these two files and pushes that
  commit to the release branch, with a commit message that deliberately
  does **not** include `[skip ci]` (the plugin's default message does) -
  GitHub applies skip directives to the tag's `push` event too, since the
  tag points at this same commit; a skip marker here would silently
  suppress `release-publish.yaml`
- Modified files: `install/overlays/default/kustomization.yaml`, `common-server/helm/Chart.yaml`
- No `@semantic-release/changelog` or `@semantic-release/release-notes-generator`
  plugin is used: the changelog lives in the GitHub Release generated by
  GoReleaser instead of a committed `CHANGELOG.md`
- **Why `@semantic-release/git` is used here, despite semantic-release's
  general advice to avoid it:** `kustomization.yaml`'s `ref=` value is
  self-referential - it points at the very tag being cut, so the *tagged
  commit's own tree* must already contain the bump. semantic-release *core*
  (not the git plugin) creates and pushes the release tag itself, pointing
  at whatever commit is HEAD after `prepare` - if that commit is never
  pushed to the release branch, it becomes unreachable from it. The *next*
  release run determines the "last release" by looking at tags reachable
  from the current branch, so an unreachable tag would be invisible to it -
  it would recompute from the older tag and could attempt to recreate the
  same version/tag again. Pushing this commit to the branch keeps the tag
...
└─ Semantic Release (analyze commits, bump version-reference files)
