...
var _ = Describe("Rover Handler", func() {
	...
	Context("Secret Manager Integration", func() {
		...
		It("should handle errors from Secret Manager", func() {
			...
			testutil.ExpectConditionToBeFalse(NewGomegaWithT(GinkgoT()), meta.FindStatusCondition(roverObj.GetConditions(), condition.ConditionTypeReady), condition.ReasonDeletionFailed)
		})
	})
})
