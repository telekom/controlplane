...
func (a AdapterSender) ProcessNotification(ctx context.Context, channel *notificationv1.NotificationChannel, subject, body string, attachments []adapter.Attachment) error {
	...
	case notificationv1.NotificationTypeMail:
		err = a.MailAdapter.Send(ctx, channel.Spec.Email, subject, body, attachments)
	case notificationv1.NotificationTypeChat:
		err = a.ChatAdapter.Send(ctx, channel.Spec.MsTeams, subject, body, attachments)
	case notificationv1.NotificationTypeCallback:
		err = a.CallbackAdapter.Send(ctx, channel.Spec.Webhook, subject, body, attachments)
	default:
