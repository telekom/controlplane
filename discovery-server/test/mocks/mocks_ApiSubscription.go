// Copyright 2025 Deutsche Telekom IT GmbH
//
// SPDX-License-Identifier: Apache-2.0

package mocks

import (
	"github.com/onsi/ginkgo/v2"
	"github.com/stretchr/testify/mock"

	apiv1 "github.com/telekom/controlplane/api/api/v1"
	"github.com/telekom/controlplane/common-server/pkg/store"
	csmocks "github.com/telekom/controlplane/common-server/test/mocks"
)

func NewAPISubscriptionStoreMock(testing ginkgo.FullGinkgoTInterface) store.ObjectStore[*apiv1.ApiSubscription] {
	mockStore := csmocks.NewMockObjectStore[*apiv1.ApiSubscription](testing)
	ConfigureAPISubscriptionStoreMock(testing, mockStore)
	return mockStore
}

func ConfigureAPISubscriptionStoreMock(testing ginkgo.FullGinkgoTInterface, mockedStore *csmocks.MockObjectStore[*apiv1.ApiSubscription]) {
	apiSubscription := GetApiSubscription(testing, apiSubscriptionFileName)

	mockedStore.EXPECT().Get(
		mock.AnythingOfType("*context.valueCtx"),
		mock.AnythingOfType("string"),
		mock.Anything,
	).Return(apiSubscription, nil).Maybe()

	mockedStore.EXPECT().List(
		mock.AnythingOfType("*context.valueCtx"),
		mock.Anything,
	).Return(
		&store.ListResponse[*apiv1.ApiSubscription]{
			Items: []*apiv1.ApiSubscription{apiSubscription},
		}, nil).Maybe()
}
