...
func (_q *ApprovalQuery) QueryAPISubscription() *ApiSubscriptionQuery {
...
func (_q *ApprovalQuery) QueryListener() *ListenerQuery {
...
func (_q *ApprovalQuery) WithAPISubscription(opts ...func(*ApiSubscriptionQuery)) *ApprovalQuery {
	query := (&ApiSubscriptionClient{config: _q.config}).Query()
...
func (_q *ApprovalQuery) WithListener(opts ...func(*ListenerQuery)) *ApprovalQuery {
	query := (&ListenerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApprovalQuery) prepareQuery(ctx context.Context) error {
	...
	if approval.Policy == nil {
		return errors.New("ent: uninitialized approval.Policy (forgotten import ent/runtime?)")
	}
	if err := approval.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *ApprovalQuery) loadAPISubscription(ctx context.Context, query *ApiSubscriptionQuery, nodes []*Approval, init func(*Approval), assign func(*Approval, *ApiSubscription)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Approval)
	for i := range nodes {
		if nodes[i].api_subscription_approval == nil {
			continue
		}
		fk := *nodes[i].api_subscription_approval
...
func (_q *ApprovalQuery) loadEventSubscription(ctx context.Context, query *EventSubscriptionQuery, nodes []*Approval, init func(*Approval), assign func(*Approval, *EventSubscription)) error {
	...
	nodeids := make(map[int][]*Approval)
	for i := range nodes {
		if nodes[i].event_subscription_approval == nil {
			continue
		}
		fk := *nodes[i].event_subscription_approval
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
...
func (_q *ApprovalQuery) loadListener(ctx context.Context, query *ListenerQuery, nodes []*Approval, init func(*Approval), assign func(*Approval, *Listener)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Approval)
	for i := range nodes {
		if nodes[i].listener_provider_approval == nil {
			continue
		}
		fk := *nodes[i].listener_provider_approval
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
