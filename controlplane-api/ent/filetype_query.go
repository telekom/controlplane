...
func (_q *FileTypeQuery) QueryExposures() *FileExposureQuery {
...
func (_q *FileTypeQuery) QuerySubscriptions() *FileSubscriptionQuery {
...
func (_q *FileTypeQuery) WithExposures(opts ...func(*FileExposureQuery)) *FileTypeQuery {
	query := (&FileExposureClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *FileTypeQuery) WithSubscriptions(opts ...func(*FileSubscriptionQuery)) *FileTypeQuery {
	query := (&FileSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(filetype.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.FileType.Query().
//		Select(filetype.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *FileTypeQuery) prepareQuery(ctx context.Context) error {
	...
	if filetype.Policy == nil {
		return errors.New("ent: uninitialized filetype.Policy (forgotten import ent/runtime?)")
	}
	if err := filetype.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *FileTypeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*FileType, error) {
	...
	if query := _q.withExposures; query != nil {
		if err := _q.loadExposures(ctx, query, nodes,
			func(n *FileType) { n.Edges.Exposures = []*FileExposure{} },
			func(n *FileType, e *FileExposure) { n.Edges.Exposures = append(n.Edges.Exposures, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubscriptions; query != nil {
		if err := _q.loadSubscriptions(ctx, query, nodes,
			func(n *FileType) { n.Edges.Subscriptions = []*FileSubscription{} },
			func(n *FileType, e *FileSubscription) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedExposures {
		if err := _q.loadExposures(ctx, query, nodes,
			func(n *FileType) { n.appendNamedExposures(name) },
			func(n *FileType, e *FileExposure) { n.appendNamedExposures(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedSubscriptions {
		if err := _q.loadSubscriptions(ctx, query, nodes,
			func(n *FileType) { n.appendNamedSubscriptions(name) },
			func(n *FileType, e *FileSubscription) { n.appendNamedSubscriptions(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *FileTypeQuery) loadExposures(ctx context.Context, query *FileExposureQuery, nodes []*FileType, init func(*FileType), assign func(*FileType, *FileExposure)) error {
	...
	query.Where(predicate.FileExposure(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(filetype.ExposuresColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.file_type_exposures
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_type_exposures" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_type_exposures" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *FileTypeQuery) loadSubscriptions(ctx context.Context, query *FileSubscriptionQuery, nodes []*FileType, init func(*FileType), assign func(*FileType, *FileSubscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*FileType)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.FileSubscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(filetype.SubscriptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.file_type_subscriptions
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_type_subscriptions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_type_subscriptions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *FileTypeQuery) WithNamedExposures(name string, opts ...func(*FileExposureQuery)) *FileTypeQuery {
	query := (&FileExposureClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedExposures == nil {
		_q.withNamedExposures = make(map[string]*FileExposureQuery)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *FileTypeQuery) WithNamedSubscriptions(name string, opts ...func(*FileSubscriptionQuery)) *FileTypeQuery {
	query := (&FileSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
