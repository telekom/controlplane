...
type ApprovalRequest struct {
	...
	api_subscription_approval_request *int
...
func (*ApprovalRequest) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case approvalrequest.ForeignKeys[0]: // api_subscription_approval_request
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case approvalrequest.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field api_subscription_approval_request", value)
			} else if value.Valid {
				_m.api_subscription_approval_request = new(int)
				*_m.api_subscription_approval_request = int(value.Int64)
			}
		default:
