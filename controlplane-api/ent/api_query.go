...
func (_q *APIQuery) QueryOwner() *TeamQuery {
...
func (_q *APIQuery) WithOwner(opts ...func(*TeamQuery)) *APIQuery {
	query := (&TeamClient{config: _q.config}).Query()
...
func (_q *APIQuery) loadOwner(ctx context.Context, query *TeamQuery, nodes []*Api, init func(*Api), assign func(*Api, *Team)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_apis" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *APIQuery) loadExposures(ctx context.Context, query *ApiExposureQuery, nodes []*Api, init func(*Api), assign func(*Api, *ApiExposure)) error {
	...
	for _, n := range neighbors {
		fk := n.api_exposures
		if fk == nil {
			return fmt.Errorf(`foreign-key "api_exposures" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "api_exposures" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
