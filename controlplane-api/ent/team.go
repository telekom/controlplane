...
func (_m *Team) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case team.FieldTeamToken:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field team_token", values[i])
			} else if value.Valid {
				_m.TeamToken = new(string)
				*_m.TeamToken = value.String
			}
...
func (_m *Team) String() string {
	...
	builder.WriteString(", ")
	if v := _m.TeamToken; v != nil {
		builder.WriteString("team_token=")
		builder.WriteString(*v)
	}
	builder.WriteByte(')')
