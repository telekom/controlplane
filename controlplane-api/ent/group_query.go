...
func (_q *GroupQuery) QueryTeams() *TeamQuery {
...
func (_q *GroupQuery) WithTeams(opts ...func(*TeamQuery)) *GroupQuery {
	query := (&TeamClient{config: _q.config}).Query()
...
//
//	client.Group.Query().
//		GroupBy(group.FieldEnvironment).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withTeams; query != nil {
		if err := _q.loadTeams(ctx, query, nodes,
			func(n *Group) { n.Edges.Teams = []*Team{} },
			func(n *Group, e *Team) { n.Edges.Teams = append(n.Edges.Teams, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedTeams {
		if err := _q.loadTeams(ctx, query, nodes,
			func(n *Group) { n.appendNamedTeams(name) },
			func(n *Group, e *Team) { n.appendNamedTeams(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadTeams(ctx context.Context, query *TeamQuery, nodes []*Group, init func(*Group), assign func(*Group, *Team)) error {
	...
	query.Where(predicate.Team(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.TeamsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_teams
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_teams" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_teams" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *GroupQuery) WithNamedTeams(name string, opts ...func(*TeamQuery)) *GroupQuery {
	query := (&TeamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedTeams == nil {
		_q.withNamedTeams = make(map[string]*TeamQuery)
	}
	_q.withNamedTeams[name] = query
	return _q
}
