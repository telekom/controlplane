...
func NamespaceIsNil() predicate.ApiSubscription {
	return predicate.ApiSubscription(sql.FieldIsNull(FieldNamespace))
}

// NamespaceNotNil applies the NotNil predicate on the "namespace" field.
func NamespaceNotNil() predicate.ApiSubscription {
	return predicate.ApiSubscription(sql.FieldNotNull(FieldNamespace))
}
...
// HasApprovalRequest applies the HasEdge predicate on the "approval_request" edge.
func HasApprovalRequest() predicate.ApiSubscription {
	return predicate.ApiSubscription(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasApprovalRequestWith applies the HasEdge predicate on the "approval_request" edge with a given conditions (other predicates).
func HasApprovalRequestWith(preds ...predicate.ApprovalRequest) predicate.ApiSubscription {
	return predicate.ApiSubscription(func(s *sql.Selector) {
		step := newApprovalRequestStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
