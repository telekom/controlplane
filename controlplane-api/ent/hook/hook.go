...
// The EventExposureFunc type is an adapter to allow the use of ordinary
// function as EventExposure mutator.
type EventExposureFunc func(context.Context, *ent.EventExposureMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f EventExposureFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.EventExposureMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.EventExposureMutation", m)
}
