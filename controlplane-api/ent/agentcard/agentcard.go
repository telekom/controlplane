...
const (
	// Label holds the string label denoting the agentcard type in the database.
	Label = "agent_card"
...
// ForeignKeys holds the SQL foreign-keys that are owned by the "agent_cards"
...
func StatusPhaseValidator(sp StatusPhase) error {
	...
	case StatusPhaseReady, StatusPhasePending, StatusPhaseError, StatusPhaseUnknown:
		return nil
	default:
		return fmt.Errorf("agentcard: invalid enum value for status_phase field: %q", sp)
	}
}

// OrderOption defines the ordering options for the AgentCard queries.
