...
// QueryExposedEvents chains the current query on the "exposed_events" edge.
func (_q *ApplicationQuery) QueryExposedEvents() *EventExposureQuery {
...
// QuerySubscribedEvents chains the current query on the "subscribed_events" edge.
func (_q *ApplicationQuery) QuerySubscribedEvents() *EventSubscriptionQuery {
...
// WithExposedEvents tells the query-builder to eager-load the nodes that are connected to
// the "exposed_events" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithExposedEvents(opts ...func(*EventExposureQuery)) *ApplicationQuery {
	query := (&EventExposureClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withExposedEvents = query
	return _q
}

// WithSubscribedEvents tells the query-builder to eager-load the nodes that are connected to
// the "subscribed_events" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithSubscribedEvents(opts ...func(*EventSubscriptionQuery)) *ApplicationQuery {
	query := (&EventSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := _q.withExposedEvents; query != nil {
		if err := _q.loadExposedEvents(ctx, query, nodes,
			...
			func(n *Application, e *EventExposure) { n.Edges.ExposedEvents = append(n.Edges.ExposedEvents, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSubscribedEvents; query != nil {
		if err := _q.loadSubscribedEvents(ctx, query, nodes,
			func(n *Application) { n.Edges.SubscribedEvents = []*EventSubscription{} },
	...
	for name, query := range _q.withNamedExposedEvents {
		if err := _q.loadExposedEvents(ctx, query, nodes,
			func(n *Application) { n.appendNamedExposedEvents(name) },
			func(n *Application, e *EventExposure) { n.appendNamedExposedEvents(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedSubscribedEvents {
		if err := _q.loadSubscribedEvents(ctx, query, nodes,
...
func (_q *ApplicationQuery) loadExposedEvents(ctx context.Context, query *EventExposureQuery, nodes []*Application, init func(*Application), assign func(*Application, *EventExposure)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Application)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.EventExposure(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.ExposedEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.application_exposed_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_exposed_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "application_exposed_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ApplicationQuery) loadSubscribedEvents(ctx context.Context, query *EventSubscriptionQuery, nodes []*Application, init func(*Application), assign func(*Application, *EventSubscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Application)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.EventSubscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.SubscribedEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.application_subscribed_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "application_subscribed_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
// WithNamedExposedEvents tells the query-builder to eager-load the nodes that are connected to the "exposed_events"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithNamedExposedEvents(name string, opts ...func(*EventExposureQuery)) *ApplicationQuery {
	query := (&EventExposureClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedExposedEvents == nil {
		_q.withNamedExposedEvents = make(map[string]*EventExposureQuery)
	}
	_q.withNamedExposedEvents[name] = query
	return _q
}

// WithNamedSubscribedEvents tells the query-builder to eager-load the nodes that are connected to the "subscribed_events"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ApplicationQuery) WithNamedSubscribedEvents(name string, opts ...func(*EventSubscriptionQuery)) *ApplicationQuery {
	query := (&EventSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedSubscribedEvents == nil {
